...
def make_dayarray(T_df):
    ...
    for no in T_No_set:
        ...
        if( len(strong) >= 5 ):
            ...
            target_list.append(target_date)
        else:
            print("Process is Skipped!")

    return target_list

list = make_dayarray(df) #階級4or5の台風の抽出 
#%%
T_No_list = df.loc[:, "台風番号"].values
T_No_set = set(df.loc[:, "台風番号"].values)
...
def make_6days(T_Date):
    ...
    """
    start_date = dt.datetime.strptime(T_Date, '%Y%m%d%H')
...
def make_data(no, daylist): 
    ...
    for i in range(len(daylist) - 6 ):
        six_days = make_6days(daylist[i, 1])

        if( all(six_days = daylist[i:i+6, 1] )):
            input_date.append(six_days)

        img_list = []
        for date in input_date:
            file_path, _ = check_file(no, date)
            img_list.apnned(file_path)
        input_img.appned(img_list)
...
test_date, test_img = make_data(list)
